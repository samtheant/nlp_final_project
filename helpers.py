...
def prepare_dataset_nli(examples, tokenizer, max_seq_length=None, hypothesis_only=False):
    ...
    # examples['hypothesis'] = [examples["hypothesis"][i] for i in range(len(examples['hypothesis'])) if examples['label'] == 2]
    # examples['premise'] = [examples["premise"][i] for i in range(len(examples['premise'])) if examples['label'] == 2]
    # examples['label'] = [ex for ex in examples['label'] if ex == 2]
    # # quit()
    if hypothesis_only:
